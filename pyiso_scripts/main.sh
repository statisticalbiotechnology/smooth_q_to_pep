...
SEED=1
OUTPUT_DIR="output" # pyiso_test/PXD003868/run1
if [ ! -z "$2" ]; then
    SEED=$2
fi
if [ ! -z "$3" ]; then
...
INTERM_DIR="$WORK_DIR/$OUTPUT_DIR/revised_files"
...
DATA_INTERM_DIR="/data/$OUTPUT_DIR/revised_files"

cd "$WORK_DIR" || { echo "Cannot change to directory $WORK_DIR"; exit 1; }
# Create necessary directories
mkdir -p "$OUTPUT_DIR" "$INTERM_DIR" "$MASTER_DIR" "$MASTER_DIR/0" "$QPAVA_DIR" "$QIPPAVA_DIR" "$QISPLINE_DIR" "$DPAVA_DIR" "$DISPLINE_DIR"
...
if [ ! -f "$INTERM_DIR/inserted.features.pin" ]; then
    ...
    podman run --rm -v "$WORK_DIR:/data" percolator:isotonic \
        ...
        "$DATA_OUTPUT_DIR/../make-pin.pin" 2>&1 | tee "$MASTER_DIR/0/percolator.log"
    ...
    python "$SCRIPTS_DIR/insert_default_direction.py" \
        ...
        --features_pin "$WORK_DIR/$OUTPUT_DIR/../make-pin.pin" \
...
for KNOCKING_OUT_NUM in 0 "${KNOCKING_OUT_NUM_LIST[@]}"; do
    ...
    podman run --rm -v "$WORK_DIR:/data" pyisotonicpep:main q2pep \
        --input "$DATA_INTERM_DIR/peptide.target.$KNOCKING_OUT_NUM.txt" \
    ...
    podman run --rm -v "$WORK_DIR:/data" pyisotonicpep:main d2pep \
        --target-file "$DATA_INTERM_DIR/peptide.target.$KNOCKING_OUT_NUM.txt" \
