...
mkdir -p "$OUTPUT_DIR" "$MASTER_DIR" "$ISOTONIC_DIR" "$IP_ISOTONIC_DIR" "$MASTER_NORMAL_DIR" "$REVISED_FEATURES_DIR"
...
for KNOCKING_OUT_NUM in "${KNOCKING_OUT_NUM_LIST[@]}"; do
    ...
    mkdir -p "$CURRENT_MASTER_DIR" "$CURRENT_ISOTONIC_DIR" "$CURRENT_IP_ISOTONIC_DIR"
