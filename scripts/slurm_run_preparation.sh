...
#SBATCH --output=/proj/proteoforma_nsc/smooth_q_to_pep/perc_test/logs/pyiso_%A_%a.out
#SBATCH --error=/proj/proteoforma_nsc/smooth_q_to_pep/perc_test/logs/pyiso_%A_%a.err
...
cd /proj/proteoforma_nsc/smooth_q_to_pep/scripts || exit 1
...
KNOCKING_OUT_NUM_LIST="1,2,3,4,5,6,7,8,20,40"
OUT_DIR="preparation"

echo "[INFO] Running dataset $DATASET with seed $SEED (TASK_ID=$TASK_ID)"

# Call the main workflow script.
bash /proj/proteoforma_nsc/smooth_q_to_pep/scripts/preparation.sh "$KNOCKING_OUT_NUM_LIST" "$OUT_DIR" "$DATASET" "$SEED"
